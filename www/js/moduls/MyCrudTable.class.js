function MyCrudTable(
    ...
    this.setAddBtnDisabled = function () {
    ...
    this.addSelectOptions = function (arr, colName) {
        var that = this;
        MyCrudTable.buildComboManual(arr, function (elem) {
    ...
    this.addSelectOptionsRest = function (rest, searchParams, colName) {
        var that = this;
        MyCrudTable.buildComboRest(rest, searchParams, colName, function (elem) {
            that.newElemMap[colName] = elem;
        });
    };
    ...
    this.create = function () {
        ...
        this.buildCreateInput(function (input) {
            ...
            MYMODALS.showInputModalB("Create new", "", input, 'lg', function (modalInput) {
                if (!modalInput) {
                    return;
                }
                ...
                $(that.fieldsArr).each(function (i, colName) {
                    if (modalInput.find("#" + colName).is('input')) {
                        //
                        var val = modalInput.find("#" + colName).val();
                        //
                        if (val) {
                            updateSettings[colName] = val;
                        }
                        //
                        ...
                        if (modalInput.find("#" + colName).children('.special-input').length > 0) {
                            //
                            var val = modalInput.find("#" + colName).find('select').val();
                            //
                            if (val) {
                                updateSettings[colName] = val.trim();
                            }
                            //
                        }
                    }

                    if (i === that.fieldsArr.length - 1) {
    ...
    this.delete = function (_id) {
        ...
        MYMODALS.showConfirmModal("OBS!", "Bekräfta handling", 'lg', 'error', function (yes) {
            if (!yes) {
                return;
            }
    ...
    MyCrudTable.prototype.edit = function (_id, colName, value) {
        var that = this;
        //
        var updateSetting = {};
        var input;
        ...
        if (that.newElemMap[colName]) {
            ...
            input = $("<textarea rows='12' cols='70' class='text-input'>" + value + "</textarea>");
        }
        //
        //
        MYMODALS.showCrudEditDeleteModal("Edit/Delete", "", input, 'lg', function (modalInput) {
            //
            if (modalInput === false) {
                return;
            }
    ...
    MyCrudTable.prototype.transformTable = function () {
        var that = this;
        //
        ...
        $(tr_arr).each(function (i, tr) {
            ...
            $(th_arr).each(function (x, th) {
                ...
                //
                //
                that.transformTable_help_one(td);
                //
                //
    ...
    MyCrudTable.prototype.transformTable_help_one = function (td) {
        //NOTHING, THIS METHOD IS FOR EXTENDING
    };
