...
module.exports = class RestrouterP {
    ...
    respond(method, query, res) {
        ...
        //
        //Usage ex:
        //var properties = {_fields: '', _distinct: "link"}; //_sort: 'link', _skip: 0, _limit: 0 -> this will not be taken into account even if will be in properties
        if (!query._distinct) {
            m = this._class[method](
                    // Mongoose query
                    query,
                    // Fields
                    _fields,
        ...
        this.app.put(this.baseRoute + 'find/*', function (req, res) {
            ...
            that.rights(req, res, function (ret) {
                if (ret) {
                    ...
                    var searchObj;
                    eval('searchObj = ' + searchStr);
                    _class.update(searchObj, req.body, {upsert: true}, function (err, result) {
                        res.json(err || result);
                    });
                }
            });
...
//             }); 
    ...
    rights(req, res, cb) {
        ...
        //
        // Important, level 0 means no Login needed
        if (level === undefined) {
            level = 0;
        }
        //
